...
- 커널 컴파일
    - 모듈
        - 예전에는 지원할 하드웨어가 많지 않아 커널이 가벼웠다.
        - 그러나 점점 많아지고 하드웨어를 제어하는 코드가 점점 무거워지게 되었고 이에 따라 자원을 많이 사용하게 됨
        - 하지만, 실제 어떤 코드는 필요시에만 가끔 사용하는 경우가 많았다. 따라서, 이러한 코드를 커널에 넣지 않고 별도로 보관했다가 필요시 호출해 사용하면 커널을 그리 무겁지 않게 하면서 더 많은 하드웨어를 지원할 수 있을 것이라 생각하였따.
        - 이에 따라 필요시마다 호출해 사용하는 코드가 바로 모듈
        - 이 경우, 커널에 넣어야 할 코드와 모듈로 분리해야 할 코드가 CentOS에 설정되어 있다.
        - 그런데, 이 코드가 과연 최적하게 내 하드웨어에 맞게 분리되어 있을까? 그렇지 않다.
        - 따라서, 이럴 때 최선 버전으로 커널을 업그레이드 하지 않고, 사용자가 임의로 지정해 기존의 커널과 똑같은 버전을 다시 컴파일할 수 있다.
        - 이러면 더 효율적인 성능이 가능하다.
        - 또한 커널은 최근 출시된 하드웨어를 장착했을 시, 기존의 커널은 지원하지 않지만 최신은 지원하는 경우도 있어, 이런 경우에는 최신 커널을 다운로드해 업그레이드해야 한다.
        - <a href="http://www.kernel.org">참조</a>
    - 커널 컴파일
        - 커널 또한 파일이며, CentOS 7의 커널은 /boot/vmlinuz-3.10.0-123.el7.x86_64이다.
        - 따라서 기존의 모듈은 /lib/modules/3.10.0-123.el7.x86_64에 있다.
        - `uname -r` : 설치된 커널 버전 확인 가능
        - 최신 커널 버전은 <a href="https://www.kernel.org/">여기</a>에서 알 수 있다.
        - 커널 컴파일은 개념도 어렵고 설정도 많이 필요함. 넘어가도록 한다.
